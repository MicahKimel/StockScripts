Volume_RSI <- RSI_Data %>% 
  ...
  mutate(GainOrLoss = case_when(
    `Volume` > lag(`Volume`, k = 1) ~ TRUE,
    TRUE ~ FALSE)) %>% 
  mutate(Gain = ifelse(GainOrLoss, (`Volume` / lag(`Volume`, k = 1))-1, 0)) %>% 
  mutate(Loss = ifelse(!GainOrLoss, 1-(`Volume` / lag(`Volume`, k = 1)), 0)) %>% 
  ...
  mutate(RSIunder30 = ifelse(RSIWeighted < 30 & lag(RSIWeighted, 1) < 40 & lag(RSIWeighted) < 50 & lag(RSIWeighted, 3) < 50
                             & lag(RSIWeighted, 4) < 50 & lag(RSIWeighted, 5) < 50
                             , return50, 0)) %>% 
  mutate(BuySignal = ifelse(RSIWeighted < 30 & lag(RSIWeighted, 1) < 40 & lag(RSIWeighted, 2) < 50 & lag(RSIWeighted, 3) < 50
                            & lag(RSIWeighted, 4) < 50 & lag(RSIWeighted, 5) < 50
                            , TRUE, FALSE)) %>% 
  mutate(RSIunder30 = ifelse(is.na(RSIunder30), 0, RSIunder30))

Volume_RSI_Df <- as.data.frame(Volume_RSI) %>% 
  group_by(Ticker) %>% 
  ...
  filter(!is.na(return50)) %>% 
  mutate(s = sum(return50)) %>% 
  add_count() %>% 
  mutate(Avg = as.numeric(s/n)) %>% 
  filter(Avg > 0.05) %>% 
  filter(n > 2000) %>% 
